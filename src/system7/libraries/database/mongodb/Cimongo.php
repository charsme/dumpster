...
class Cimongo extends Cimongo_extras
{
    ...
    public $debug = false;
        ...
        public function insert($collection = "", $insert = array())
        {
            ...
            if (count($insert) == 0) { //echo count($insert)."<br>";
                        show_error("Nothing to insert into Mongo collection or insert is not an array", 500);
            }
            $this->_inserted_id = false;
            try {
                $query = $this->db->selectCollection($collection)->insert($insert); //var_dump($query);
                        if (isset($insert['_id'])) {
        ...
        public function insert_batch($collection = "", $insert = array())
        {
            ...
            try {
                $query = $this->db->selectCollection($collection)->batchInsert($insert);
                if (is_array($query)) {
        ...
        public function update($collection = "", $data = array(), $options = array())
        {
            ...
            try {
                $options = count($options)>0 ? $options : array("upsert"=>true);
