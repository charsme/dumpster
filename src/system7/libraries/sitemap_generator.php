<?php
/**
 * Vemale Sitemap Generator
 * Author : Doni Portnoy (22 03 2012)
 * Input  : Array
 * Output : XML
 * - Output yang dikeluarkan didalam file .php jangan lupa ditambahkan header  - header("Content-type:text/xml; charset=utf-8") - di
 *   awal line
 *
 * Fungsi write to xml file : disable
 * */
...
class xml_sitemap_entry
{
    ...
    public function __construct($loc, $priority, $changefreq="", $lastmod='', $mixedValue = array())
    {
        $this->_setLoc($loc);
        ...
        if (is_array($mixedValue)) {
            $this->_setMixedValue($mixedValue);
        }
    }
    ...
    private function _setMixedValue($mixedValue)
    {
        $this->_mixedValue = (isset($mixedValue[0]))?$mixedValue[0] : array();
        return $this;
    }
    ...
    private function _getMixedValue()
    {
        return $this->_mixedValue;
    }
}
...
xmlns:news="http://www.google.com/schemas/sitemap-news/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">*/
    ...
    private function _buildEntry(xml_sitemap_entry $entry)
    {
        ...
        return sprintf("<url>\n%s%s%s%s%s</url>\n",
                ...
                $this->_buildLine('', $entry->get('mixedValue')));
    }
